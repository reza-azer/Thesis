...
brightness = 50  # Rentang -100 hingga 100
contrast = 60    # Rentang -100 hingga 100

'''

# Membuat direktori penyimpanan gambar dan data jika belum ada
base_folder = 'CapturedIMG'
...
try:
    while True:
        ...
        ret, frame = cap.read()
        img = cv2.convertScaleAbs(frame, alpha=(contrast / 50 + 1), beta=brightness)
        ...
        _, binary = cv2.threshold(gray, 21, 255, cv2.THRESH_BINARY)
        ...
        for (x, y, w, h) in faces:
            ...
            inner_x, inner_y, inner_w, inner_h = x + 20, y + 30, w - 40, h - 90
            ...
            ix, iy, iw, ih = x + 20, y + 30, w - 40, h - 90
            ic = img [iy:iy+ih, ix:ix+iw]
            gc = gray[iy:iy+ih, ix:ix+iw]
            bc = binary[iy:iy+ih, ix:ix+iw]
            data = (bc / 255).astype(int)

            cv2.putText(img, f'resolusi : {width} x {height}', (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

            # Kondisi ukuran bounding box untuk penentuan jarak
            if 130 <= w <= 180 and 130 <= h <= 180:
            ...
            cv2.imshow('main', img, )
            facetrackV2.track_face(frame, x, y, w, h)
